...
(defun my-org-clipboard-func()
  (interactive)
  (let*
    ((file-dir (concat default-directory "img/"))
     (file-name (format-time-string "screenshot-%Y%m%d-%H%M%S.png"))
     (file-path (concat file-dir file-name))
     (command (cond  ;按照不同的操作系统生成shell命令字符串
		   ...
		   (concat "c:/Windows/System32/WindowsPowerShell/v1.0/powershell.exe"
			   ...
			   "\\\")\""))
		  (t nil))))
    ;如果command不是nil则执行后续代码；否则该操作系统的行为未定义
    (if (eq command nil)
	(message "have not define the shell command string on this OS: " system-type)
      (progn
	  (when (not (file-exists-p file-dir))
	    (make-directory file-dir))
	  (message command)
	  (if (eq (shell-command command) 0)
	    ;文件已经被正确的拷贝，此时插入org图片相关字符
	    (progn
	      (insert (concat "#+ATTR_ORG: :width 800\n"))
            (org-indent-line)
            (insert (concat "[[file:./img/" file-name "]] ")))
	    (message "can not copy clipboard image into ./img/ folder!"))))))


(add-hook 'org-mode-hook '(lambda ()
                            (local-set-key (kbd "C-S-y") 'my-org-clipboard-func)))
